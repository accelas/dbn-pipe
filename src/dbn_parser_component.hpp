...
void DbnParserComponent<D>::DrainBuffer() {
    ...
    while (!suspended_ && HasCompleteRecord()) {
        ...
        // aligned_alloc requires: alignment is power of 2 AND multiple of sizeof(void*)
        // Use max of alignof(max_align_t) and sizeof(void*) for portability
        ...
        static_assert(kMaxBufferSize <= SIZE_MAX - alignment,
            "kMaxBufferSize too large for safe alignment rounding");
